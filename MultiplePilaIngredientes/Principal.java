...
quitarIngrPlatilloCosto(ColaSimplePlatillo cp, MultiPilaIngr mpi, double costox){
	int np <- cp.nroElem();
	for(i <- 0; i < np; i++) {
		Platillo x <- cp.eli();
		if(x.getCosto() > costox ) {
			
			//Primera etapa de encontrar la cantidad mayor
			int mayCantIngr <- buscarMayorCantIngre(mpi, i);
			//Segunda etapa quitar(eliminar) los ingredeientes con mayor cantidad
			aux ,- new PilaIngrediente();
			while(!mpi.esVacia(i)) {	
				Ingrediente x <- mpi.eliminar(i);
				if(x.getCantidad() != mayCantIngr) {
					aux.adi(x);
				}
...
//puede ser int o double
int buscarMayorCantIngre(MultiPilaIngr mpi, int i) {
	aux <- new PilaIngredientes();
	int may <- 0;
	while(!mpi.esVacia(i)) {
		Ingrediente x <- mpi.eliminar(i);
		if(x.getCantidad() > may) {
			may <- x.getCantidad();
		}
		aux.adi(x);
	}
